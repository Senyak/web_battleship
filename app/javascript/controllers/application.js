...
class Field {
    constructor(field, role) {
        ...
        this.fire = (target) => {
            ...
            if (userCount == 20) {
                alert('Вражеская флотилия потоплена!!!!')
        ...
        this.backFire = () => {
            ...
            if (sheeps.length === 0) {
                alert('К сожалению, ваша флотилия потоплена...')
            }
        }
    }
...
window.onload = () =>{
    ...
    var gameu = new Field(fields.at(Math.floor(Math.random() * fields.length)), 'user')
    gameu.render();

    var gamec = new Field(fields.at(Math.floor(Math.random() * fields.length)) , 'comp')
    gamec.render();
